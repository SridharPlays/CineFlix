...
const movie = movies.find(movie => movie.name.includes(movieName));
...
function loadMovieDetails(movie) {
    ...
    document.title = `${movie.name} - ${movie.age_rating}`;

    let abbrMeaning = {
        'U/A': "Unrestricted Public Exhibition - Parental Discretion",
        'U': "Universal",
        'A': "Adult Only"
    }[movie.age_rating] || "Unknown";

    let musicElement = '';
    if (movie.music) {
        musicElement = `
            <audio id="bgm" controls autoplay loop>
                <source src="./Musics/${movie.music}" type="audio/mpeg">
    ...
    moviesInfoDiv.innerHTML = `
        ...
        <header>
            <h2>${movie.name} - <abbr title="${abbrMeaning}">${movie.age_rating}</abbr></h2>
        </header>
        <div class="wrapper">
            ...
            <div class="content">
                ...
                <p class='story'>Story: ${movie.story}</p>
...
function setupEventListeners(movie) {
    ...
    // Initializing This to Mute Audio When Opening iFrame!
    ...
    document.querySelector(".image-container").addEventListener("click", () => {
        ...
        trailerContainer.innerHTML = `
            <div class="trailer">
                ...
                <iframe width="854" height="480" src="${movie.trailer}&autoplay=1&fs=1&controls=0" frameborder="0" allowfullscreen></iframe>
